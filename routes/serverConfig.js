...
serverConfigRouter.use(async (request, response, next) => {
...
configRouter.post("/", async (request, response) => {
    try {
        ...
        if (body.tomeChannel)
            newServer.tomeChannel = body.tomeChannel;
        if (body.layoffsChannel)
            newServer.layoffsChannel = body.layoffsChannel;
        if (body.raidsChannel)
            newServer.raidsChannel = body.raidsChannel;
        if (body.warChannel)
            newServer.warChannel = body.warChannel;
        if (body.broadcastingChannel)
            newServer.broadcastingChannel = body.broadcastingChannel;
        if (body.listeningChannel)
            newServer.listeningChannel = body.listeningChannel;
...
serverConfigRouter.get("/", async (request, response) => {
    try {
...
serverConfigRouter.delete("/", async (request, response) => {
    try {
...
serverConfigRouter.patch("/", async (request, response) => {
    try {
        const body = request.body;
        if (body.tomeChannel)
            request.serverConfig.tomeChannel = body.tomeChannel;
        if (body.layoffsChannel)
            request.serverConfig.layoffsChannel = body.layoffsChannel;
        if (body.raidsChannel)
            request.serverConfig.raidsChannel = body.raidsChannel;
        if (body.warChannel)
            request.serverConfig.warChannel = body.warChannel;
        if (body.broadcastingChannel)
            request.serverConfig.broadcastingChannel = body.broadcastingChannel;
        if (body.listeningChannel)
            request.serverConfig.listeningChannel = body.listeningChannel;
        if (body.privilegedRoles)
            await Promise.all([
                body.privilegedRoles.forEach((v) => {
...
serverConfigRouter.post("/privileged-role", async (request, response) => {
    try {
...
serverConfigRouter.delete("/privileged-role", async (request, response) => {
    try {
